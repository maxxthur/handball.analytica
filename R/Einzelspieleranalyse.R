Einzelspieleranalyse <- function(Player, Team, Data){
  ...
  Tore_Spiele <- Data[[1]] %>%
    dplyr::filter(Name=="RUHNKE, JOERG") %>%
    ...
    ggplot2::xlab("Gegner") +
    ggplot2::ylab("Tore") +
    ggplot2::ggtitle(label=paste("Tore ", "RUHNKE, JOERG"))
  ...
  Zeitstrafen_Spiele <- Data[[1]] %>%
    ...
    ggplot2::ggtitle(label=paste("Zeitstrafen ", "RUHNKE, JOERG"))
  ...
  Gelbe_Spiele <- Data[[1]] %>%
    ...
    ggplot2::ggtitle(label=paste("Gelbe Karten ", "RUHNKE, JOERG"))

  Torgefahr <- handball.analytica::Tore_Spielminute_Spieler("RUHNKE, JOERG", "DJK Grün-Weiß Nottuln", Data)
  Zeitstrafen <- handball.analytica::Zeitstrafen_Spielverlauf_Spieler("RUHNKE, JOERG", Data)
  Gelbe <- handball.analytica::GelbeKarten_Spielverlauf_Spieler("RUHNKE, JOERG", Data)
