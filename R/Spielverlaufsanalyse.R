...
Tore_Spielminute_Spieler <- function(Name, Team, Data) {
  Tore_Spieler <- Data[[3]] %>%
    filter(Spieler==Name) %>%
    mutate(Spielabschnitt=as.numeric(substr(Spielminute, 1,1))+1) %>%
    group_by(Datum) %>% count(Spielabschnitt)

  Tore_Mannschaft <- Data[[1]] %>%
    filter(Mannschaft==Team) %>%
    group_by(Datum) %>%
    summarise(Tore=sum(Tore))
  ...
  for(i in (1:length(unique(Tore_Spieler$Datum)))) {
    Gegn <- as.character(unique(Data[[1]] %>%
                                  filter(Datum==unique(Tore_Spieler$Datum)[i] & Mannschaft==Team) %>%
                                  .$Gegner))

    Mean_Gegentore_Gegner <- mean(Data[[1]] %>%
                                    filter(Gegner==Gegn) %>%
                                    group_by(Datum)  %>%
                                    summarise(Gegentore=sum(Tore)) %>%
                                    .$Gegentore)
    ...
    Tore_erzielt_Spieler <- Tore_Spieler %>% filter(Datum==unique(Tore_Spieler$Datum)[i])
    Tore_erzielt_Mannschaft <- Tore_Mannschaft %>% filter(Datum==unique(Tore_Spieler$Datum)[i])
