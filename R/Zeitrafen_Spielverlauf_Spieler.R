...
Zeitstrafen_Spielverlauf_Spieler <- function(Name, Data) {
  ...
  for(i in (1:nrow(Spieler_Zeitstrafen))) {
    if(Spieler_Zeitstrafen$Spielabschnitt[i]==1) {
      Enddaten$`1` <- Enddaten$`1`+1
    } else if (Spieler_Zeitstrafen$Spielabschnitt[i]==2) {
      Enddaten$`2` <- Enddaten$`2`+1
    } else if (Spieler_Zeitstrafen$Spielabschnitt[i]==3) {
      Enddaten$`3` <- Enddaten$`3`+1
    } else if (Spieler_Zeitstrafen$Spielabschnitt[i]==4) {
      Enddaten$`4` <- Enddaten$`4`+1
    } else if (Spieler_Zeitstrafen$Spielabschnitt[i]==5) {
  ...
  ggplot2::ggplot(Enddaten, ggplot2::aes(x=rownames(Enddaten), y=Enddaten$V1)) +
    ...
    ggplot2::ggtitle(label=paste("2 Minuten ", Name))
}
